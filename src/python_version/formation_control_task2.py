...
for iter in range(max_iter):
    ...
    if Jn_converged:
        ...
        x_y_diff_vector = swarm_destination - swarm_position[k]        
        distance_to_destination = np.linalg.norm(x_y_diff_vector)
        swarm_velocity = x_y_diff_vector / distance_to_destination
        
        # Calculate the scaling factor to the swarm_velocity
        scaling_factor = am if distance_to_destination > bm else am * (distance_to_destination / bm)

        for i in range(swarm_size):
            swarm_control_ui[i, 0] = swarm_velocity[0] * scaling_factor
